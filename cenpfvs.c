...
void cenpfvs()
{
  ...
  // TFile * outputfile = new TFile("tauPH.root", "recreate");
  TFile * outputfile = new TFile("tauPbPb.root", "recreate");
  ...
  for(Long64_t jentry = 0; jentry<300000; jentry++){
    ...
    for(unsigned int i =0; i<min(2,size); i++)
    {
      ...
      if(jets[i].perp() >30.0)
      {
        ...
        else if(centrality<=100.0 && centrality > 0.0){
        ...
        if(centrality<=0.0 && centrality >=0.0){
